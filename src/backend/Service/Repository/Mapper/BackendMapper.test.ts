...
describe('BackendMapper', () => {
  ...
  describe('integration tests', () => {
    test('integration: complete fuel station mapping with all utilities', async () => {
      const station = {
        Name: 'BP Test Station',
        Brand: 'BP',
        Address: '123 Test St, Sydney NSW 2000',
    ...
    test('integration: handles complex location extraction', async () => {
      const station = {
        Name: 'Metro Bonnyrigg',
        Brand: 'Metro Fuel',
        Address: '709 Cabramatta Road, Bonnyrigg NSW 2177',
      ...
      const result = await mapper.mapToFuelStation(station, mockRefData.brands.items);
      expect(result.location).toBe('Bonnyrigg');
      expect(result.address).toBe('709 Cabramatta Road, Bonnyrigg NSW 2177');
      expect(result.logoUrl).toBe('metro.png');
    });

    test('integration: handles brand mapping correctly', async () => {
      const station = {
        Name: 'Shell Test Station',
        Brand: 'Shell',
      ...
      expect(result.logoUrl).toBe('shell.png');
    });
